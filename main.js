...
let TestHost = class TestHost extends LitElement {
    ...
    onInlineKeyDown({ detail: { inline, node, keyboardEvent }, }) {
        ...
        if (keyboardEvent.key === 'ArrowUp') {
            ...
            if (result !== true) {
                const next = findNextDfs(node, ({ type }) => ['paragraph', 'code-block', 'heading'].includes(type));
                if (next)
...
function normalizeSection(node) {
    ...
    while (next) {
        ...
        if (child.type === 'heading') {
            ...
            child.viewModel.insertBefore(cast(node.viewModel.parent), node.viewModel.nextSibling);
        }
